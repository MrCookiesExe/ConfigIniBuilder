...
function addRowSmart(table) {
    ...
    <div style="display: table-cell; width: 30%;">${c.value.replace("\n", "<br>")}</div>
    <div style="display: table-cell; width: 10%;">
        <b style="cursor: pointer;" onclick="RemoveFromTable('${table}${ident}')">&times;</b>
    </div></div>`
    key.value = ""
    ...
    reader.onload = function (progressEvent) {
        ...
        for (sec in ini) {
            for (let key in ini[sec]) {
                if (key == "") continue;
                let c;
                switch (ChengeToIds(sec)) {
                    case "settings":
                        switch (key) {
                            case "regex":
                                document.getElementById(`${ChengeToIds(sec)}/${key}`).value = ini[sec][key].split(",").join("\n")
                                break;
                            default:
                                if (document.getElementById(`${ChengeToIds(sec)}/${key}`) == null)
                                    break;
                                let data = ini[sec][key].split(",")
                                document.getElementById(`${ChengeToIds(sec)}/${key}`).value = data[0]
                                if (data.length > 1) {
                                    document.getElementById(`${ChengeToIds(sec)}/${key}/where`).value = ChengeToValues(data[1])
                                }
                                break;
                        }
                        break;
                    case "Replace": case "ReplacePieces":
                        document.getElementById(`${ChengeToIds(sec)}-K`).value = key
                        document.getElementById(`${ChengeToIds(sec)}-V`).value = ini[sec][key]
                        document.getElementById(`${ChengeToIds(sec)}-Add`).click()
                        break;
                    case "Vowels":
                        document.getElementById(`${ChengeToIds(sec)}-K`).value = key
                        document.getElementById(`${ChengeToIds(sec)}-L`).value = ini[sec][key].split(",")[0]
                        document.getElementById(`${ChengeToIds(sec)}-R`).value = ini[sec][key].split(",")[1]
                        document.getElementById(`${ChengeToIds(sec)}-Add`).click()
                        break;
                    case "Consonants":
                        c = ini[sec][key].split(",")
                    ...
                    case "Smart":
                        document.getElementById(`${ChengeToIds(sec)}-K`).value = key
                        c = ini[sec][key].split(",")
                        document.getElementById(`${ChengeToIds(sec)}-D`).value = c.shift()
                        document.getElementById(`${ChengeToIds(sec)}-C`).value = c.join("\n")
                        document.getElementById(`${ChengeToIds(sec)}-Add`).click()
                        break;
                    case "Time":
                        if (key == "IsMs") {
                        ...
                        document.getElementById(`${ChengeToIds(sec)}-K`).value = key
                        c = ini[sec][key].split(",")
                        document.getElementById(`${ChengeToIds(sec)}-Mv`).value = c.shift()
                        document.getElementById(`${ChengeToIds(sec)}-T`).value = c.shift()
                        ...
                        at.forEach(a => {
                            if (c.includes(a)) {
                                document.getElementById(`${ChengeToIds(sec)}-${a}`).checked = true
                            } else {
                                document.getElementById(`${ChengeToIds(sec)}-${a}`).checked = false
                            }
                        })
                        ...
                        break;
                    default:
                        let elem = document.getElementById(`${ChengeToIds(sec)}/${key}`)
                        if (elem != null)
